...
class User(Base):
    ...
    auth_session = relationship("AuthSession", back_populates="user", uselist=False, cascade="all, delete-orphan", lazy="selectin")
    contacts = relationship("Contact", back_populates="user", cascade="all, delete", lazy="selectin")
#
# class User(Base):
#
#     __tablename__ = "users"
...
#     updated_at: Mapped[date] = mapped_column(Date, nullable=True, onupdate=func.now())
#
#     contacts_id = mapped_column(Integer, ForeignKey("contacts.id"))
#     auth_session_id = mapped_column(Integer, ForeignKey("auth_sessions.id"))
